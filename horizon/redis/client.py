...
# from horizon.storage import magic_mirror_storage
import redis
import pickle
...
class Redis(redis.Redis):
    ...
    def get_perfect_object_data(self, value):
        ...
        #                 setattr(object_data[key], 'storage', magic_mirror_storage)
        # elif issubclass(type(object_data), Model):
        #     for field in object_data._meta.fields:
        #         if issubclass(type(field), (FieldFile, FileField)):
        #             setattr(getattr(object_data, field.name), 'storage', magic_mirror_storage)
        return object_data
